                ...
                <div class="col-xl-6">
                    <div class="card border-0 shadow-sm h-100" style="border-radius: 15px;">
                        <div class="card-header bg-transparent border-0 pt-4 pb-0">
                            <h6 class="m-0 fw-bold text-primary"><i class="fas fa-chart-bar me-2"></i> 20 อันดับโรคผู้ป่วยนอก <span class="text-muted small fw-normal ms-2">ช่วงวันที่: {{ DateThai($start_date) }} ถึง {{ DateThai($end_date) }}</span></h6>
                        </div>
                        <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach ($diag_icd10 as $index => $row)
                                            <tr>
                                                ...
                                                <td><div class="text-truncate" style="max-width: 200px;" title="{{ $row->name }}">{{ $row->name }}</div></td>
                                                <td><div class="text-truncate text-muted small" style="max-width: 150px;" title="{{ $row->tname }}">{{ $row->tname }}</div></td>
                                                <td class="text-center fw-bold text-primary">{{ number_format($row->sum) }}</td>
                                    ...
                                    <tbody>
                                        @foreach ($diag_504 as $index => $row)
                                            <tr>
                                                ...
                                                <td class="text-center fw-bold text-warning">{{ number_format($row->sum) }}</td>
                                                <td class="text-center text-primary">{{ number_format($row->male) }}</td>
                                                <td class="text-center text-danger">{{ number_format($row->female) }}</td>
                                                <td class="text-end">฿{{ number_format($row->inc_lab, 2) }}</td>
                                                <td class="text-end">฿{{ number_format($row->inc_drug, 2) }}</td>
...
<script>
    $(document).ready(function() {
            ...
            return new Chart(document.getElementById(ctxId), {
                ...
                data: {
                    labels: dataArray.map(d => {
                        let text = d.tname || d.name; // Use tname if available (ICD10), else name (Group 504)
        ...
        if(icd10Data.length > 0) createBarChart('chartIcd10', 'ผู้ป่วย (คน)', icd10Data.slice(0, 20), '#4e73df');
