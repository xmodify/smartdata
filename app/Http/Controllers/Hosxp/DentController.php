...
class DentController extends Controller
{
    ...
    {
        $title = 'สถิติผู้รับบริการทันตกรรม (OPD)';
    ...
    public function top20_diag(Request $request)
    {
        ...
        $diag_top20 = DB::connection('hosxp')->select('
            ...
            AND v.vn IN (SELECT vn FROM dtmain)            
            GROUP BY v.pdx, i.name, i.tname  
            ORDER BY sum DESC 
            LIMIT 20
        ', [$start_date, $end_date]);

        return view('hosxp.dent.top20_diag', compact(
            'title', 'budget_year_select', 'budget_year', 'start_date', 'end_date',
            'diag_top20'
        ));
    }
    ...
    {
        $title = 'มูลค่าการให้บริการทันตกรรม';
        $dates = $this->resolveDateRange($request);
        $start_date = $dates['start_date'];
        $end_date = $dates['end_date'];
        $budget_year = $dates['budget_year'];
        ...
        // 1. บริการทันตกรรม (Type 12)
        $data_dent = DB::connection('hosxp')->select('
            SELECT 
                n.`name`,
                ot.icode,
                SUM(ot.qty) AS qty,
        ...
        $data_other = DB::connection('hosxp')->select('
            ...
            WHERE o.vstdate BETWEEN ? AND ?
              AND ot.rxdate BETWEEN ? AND ?
              AND o.vn IN (SELECT vn FROM dtmain)
              AND n.income NOT IN ("02", "13")
            GROUP BY ot.icode
            ORDER BY sum_price DESC
            LIMIT 100
        ', [$start_date, $end_date, $start_date, $end_date]);

        return view('hosxp.dent.service_value', compact(
            'title', 'budget_year_select', 'budget_year', 'start_date', 'end_date',
            'data_dent', 'data_inst', 'data_other'
        ));
    }
    ...
    private function resolveDateRange(Request $request)
    {
        ...
        if ($request->start_date && $request->end_date && $request->budget_year_changed != "1") {
            ...
            $matched_year = DB::table('budget_year')->where('DATE_BEGIN', '<=', $start_date)->where('DATE_END', '>=', $start_date)->value('LEAVE_YEAR_ID');
            if ($matched_year) $budget_year = $matched_year;
        } else {
